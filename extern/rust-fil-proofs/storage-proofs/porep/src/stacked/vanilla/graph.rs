...
#[inline]
fn my_read_node<'a>(i: usize, parents_data: &'a [u8]) -> &'a [u8] {
    let start = i * NODE_SIZE;
    let end = start + NODE_SIZE;
    &parents_data[start..end]
}
    ...
    G: Graph<H> + ParameterSetMetadata + Sync + Send,
{
    ...
    //添加的内容
    pub fn my_copy_parents_data_exp(
        &self,
        base_data: &[u8],
        rx_data: &bounded_spsc_queue::Consumer<[u8; 448]>,
        hasher: Sha256,
        rx_cache: &bounded_spsc_queue::Consumer<[u32; 14]>,
    ) -> Result<[u8; 32]> {
        Ok(self.my_copy_parents_data_inner_exp(rx_cache, base_data, rx_data, hasher))
    }

    fn my_copy_parents_data_inner_exp(
        &self,
        rx_cache: &bounded_spsc_queue::Consumer<[u32; 14]>,
        base_data: &[u8],
        rx_data: &bounded_spsc_queue::Consumer<[u8; 448]>,
        ...
        let cache_parents = rx_cache.pop();
        prefetch(&cache_parents[..BASE_DEGREE], base_data);
        let parents_data = rx_data.pop();
        ...
        let parents = [
            ...
            read_node(4, &cache_parents, base_data),
            read_node(5, &cache_parents, base_data),
            my_read_node(6, &parents_data),
            my_read_node(7, &parents_data),
            my_read_node(8, &parents_data),
            my_read_node(9, &parents_data),
            my_read_node(10, &parents_data),
            my_read_node(11, &parents_data),
            my_read_node(12, &parents_data),
            my_read_node(13, &parents_data),
        ];
