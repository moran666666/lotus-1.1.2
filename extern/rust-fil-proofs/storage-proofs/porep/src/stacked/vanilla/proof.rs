...
impl<'a, Tree: 'static + MerkleTreeTrait, G: 'static + Hasher> StackedDrg<'a, Tree, G> {
    ...
    fn generate_labels(
        ...
        // 添加的内容
        let mut exp_queue_length: usize = 1000;
        if !(env::var("EXP_QUEUE_LENGTH").is_err()) {
            exp_queue_length = match env::var("EXP_QUEUE_LENGTH") {
                Ok(val) => val.parse::<usize>().unwrap(),
                Err(e) => e.to_string().parse::<usize>().unwrap(),
            };
        }
        let graph_size = graph.size();
        let (tx_ctrl, rx_ctrl) = bounded_spsc_queue::make(1);
        let (tx_cache, rx_cache) = bounded_spsc_queue::make(exp_queue_length);
        let (tx_data, rx_data) = bounded_spsc_queue::make(exp_queue_length);
        let graph_clone = graph.clone(); 
        let path_string: String = config.path.as_os_str().to_str().unwrap().to_string();
        thread::spawn( move || {
            info!("start prefect thread...");

            let mut layer_file_path = PathBuf::from(path_string);
            loop { // 2~11层循环
                    let layer = rx_ctrl.pop();  // 阻塞等待每层的完成信号

                    let file_name = format!("{}{}{}", String::from("sc-02-data-layer-"), layer-1, String::from(".dat"));
                    layer_file_path.push(file_name);
                    info!("prefect last layer file: {:?}", layer_file_path);
                    let mut layer_fd = OpenOptions::new().read(true).open(&layer_file_path).expect("Unable to open file");
                    layer_fd.seek(SeekFrom::Start(0)).unwrap();
                    let exp_labels = unsafe {memmap::MmapOptions::new().map(&layer_fd).expect("Could not access data from memory mapped file")};

                    let mut cache = graph_clone.parent_cache().unwrap();
                    for fetch_node in 1..graph_size { // 1~1G个节点循环  graph_size=1073741824;实际取到1073741823节点
                        let cache_parents = cache.read(fetch_node as u32).unwrap();
                        let mut parents_data: [u8; 448] = [0; 448];                    // 448 = 14 * 32
                        for i in 6..=13 {
                            let begin = i * 32;
                            let end = begin + 32;
                            let start = cache_parents[i] as usize * 32;
                            let stop = start + 32;
                            parents_data[begin..end].copy_from_slice(&exp_labels[start..stop]);
                        }
                        tx_cache.push(cache_parents);
                        tx_data.push(parents_data);

                        if fetch_node >= graph_size {  // 循环至最后一个节点
                            loop {
                                if tx_data.free_space() == 10 { // 等待接收端完成全部接收
                                    break;
                                }
                    ...
                    layer_file_path.pop();  // 更改层名

                    if layer >= 11 {  // 完成11层退出线程
                        break;
                    }
                }
                info!("exit prefect thread...");
        });
        // 添加的内容
